...
setwd("C:/Users/nealk/OneDrive - University of Oklahoma/Research/Thesis/Data/shapefiles/tri_county/")
...
shape.parcel  <- readOGR("." , "sa_01_parcels2014")
...
shape.parcel2$residuals <- multiple.regression2$residuals

##################
## Summary Statistics
description.independent <- c("Median household age"                  , "Ratio driving alone"            , "Ratio driving carpool"       , "Average commute time (minutes)"    , "Ratio having a GED" , 
                             "Ratio having a bachelor's degree" , "Ratio having a graduate degree" , "Ratio houses that are owned" , "Ratio houses that are rented" , 
                             "Ratio houses that are occupied"   , "Age of house (years)"                , "Median household income"          , "Redline neighborhoods"             , 
...
write.csv(df.summaryStats , 
          "C:/Users/nealk/OneDrive - University of Oklahoma/Research/Thesis/Results/summaryStats2.csv" ,
          row.names = F)
...
## [Spatial Autocorrelation Scatterplot]:
shape.parcel2@data$lags <- NA

for(i in 1 : nrow(shape.parcel2)){
  neighbors.i <- neighbors[[i]]
  
  shape.parcel2@data[i , "lags"] <- mean(shape.parcel2@data[neighbors.i , "fitted"])
}

parcels <- shape.parcel2@data %>%
           select(fitted , lags , moransI : lisaTest , perGrass : meanNDVI) #%>%
           filter(lisaTest > 0)
...
labs(x = "Fitted Statistical Value" ,
     y = "Lagged Value") +
theme_bw() +
...
ggsave("C:/Users/nealk/OneDrive - University of Oklahoma/Research/Thesis/Results/presentation/globalSA_densityplot.tiff" ,
       p1 ,
       dpi = 300   ,
       width  = 10 ,
       height = 10  ,
...
ggsave("C:/Users/nealk/OneDrive - University of Oklahoma/Research/Thesis/Results/presentation/localSA_densityplot.tiff" ,
       p2 ,
       dpi = 300   ,
       width  = 10 ,
       height = 10  ,
...
ggsave("C:/Users/nealk/OneDrive - University of Oklahoma/Research/Thesis/Results/presentation/fit_v_ndvi_densityplot.tiff" ,
       p3 ,
       dpi = 300   ,
       width  = 10 ,
       height = 10  ,
